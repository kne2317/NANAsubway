...
public class ArriveTime_4 extends JFrame {
	...
	private void initialize() {
		...
		arriveScan.setForeground(new Color(0, 153, 255)); arriveScan.setFont(new Font("함초롬돋움", Font.PLAIN, 45)); arriveScan.setBounds(950, 221, 132, 66);
		ArriveTime.add(arriveScan);
		
		JLabel startScan = new JLabel("출발지");
		startScan.setForeground(new Color(0, 153, 255));
		...
		arriveScanner.setForeground(new Color(0, 153, 255));
		...
		startScanner.setForeground(new Color(0, 153, 255));
		...
		print.setForeground(new Color(0, 153, 255));
		...
		print2.setForeground(new Color(0, 153, 255));
	...
	public int Start_To_Arrive(String start, String arrive) {
		...
		for(int i = 0; i<Sub_station.station4.length; i++) {
			...
			if(start.equals(Sub_station.station4[i])) { //입력받은 출발역의 값과 역 배열 비교하여 주소값 찾기
		...
		for(int i = 0; i < Sub_station.station4.length; i++) {
			...
			if(arrive.equals(Sub_station.station4[i])) { //입력받은 도착역의 값과 역 배열 비교하여 주소값 찾기
