...
public class ArriveTime_3 extends JFrame {
	...
	private void initialize() {
		...
		arriveScan.setForeground(new Color(255, 102, 0)); arriveScan.setFont(new Font("함초롬돋움", Font.PLAIN, 45)); arriveScan.setBounds(950, 221, 132, 66);
		ArriveTime.add(arriveScan);
		
		JLabel startScan = new JLabel("출발지");
		startScan.setForeground(new Color(255, 102, 0));
		...
		arriveScanner.setForeground(new Color(204, 102, 51));
		...
		startScanner.setForeground(new Color(204, 102, 51));
		...
		subwaylogo.setIcon(new ImageIcon("3subway Icon.jpg"));
		subwaylogo.setBounds(814, 42, 400, 400);
		ArriveTime.add(subwaylogo);
		
		JLabel trainMap = new JLabel();
		trainMap.setIcon(new ImageIcon("3TrainMap2.jpg"));
		trainMap.setBounds(55, 12, 768, 426);
		ArriveTime.add(trainMap);
		
		JLabel note = new JLabel("띄어쓰기 X");
		note.setForeground(new Color(255, 102, 0));
		...
		print.setForeground(new Color(255, 153, 0));
		...
		print2.setForeground(new Color(255, 153, 0));
	...
	public int Start_To_Arrive(String start, String arrive) {
		...
		for(int i = 0; i<Sub_station.station3.length; i++) {
			...
			if(start.equals(Sub_station.station3[i])) { //입력받은 출발역의 값과 역 배열 비교하여 주소값 찾기
		...
		for(int i = 0; i < Sub_station.station3.length; i++) {
			...
			if(arrive.equals(Sub_station.station3[i])) { //입력받은 도착역의 값과 역 배열 비교하여 주소값 찾기
